@forelse ($dataSearch as $item)
    <tr>
                ...
                @if ($is_kredit_page)
                    @if (!$item->tgl_ketersediaan_unit)
                        <button class="toggle-modal-atur-ketersediaan underline text-red-600"
                            data-target-id="modalAturKetersedian" data-id_kkb="{{ $item->kkb_id }}"
                            onclick="showModal(this)">
                            Atur
                        </button>
                    @else
                    ...
                    @if ($item->invoice)
                        @if ($item->invoice->is_confirm)
                            <a class="m-0 tagihan-modal toggle-modal-tagihan"
                                style="cursor: pointer; text-decoration: underline;" data-target-id="modalTagihan"
                                data-id="{{ $item->id }}" data-kategori="{{ $item->kategori }}"
                                data-file="{{ $item->invoice->file }}" data-confirm="{{ $item->invoice->is_confirm }}"
                    ...
                    @else
                        ...
                        @if ($is_kredit_page)
                            <button class="toggle-modal-upload-berkas-tagihan underline text-red-600"
                                data-target-id="modalUploadBerkasTagihan" data-id_kkb="{{ $item->kkb_id }}"
                                onclick="showModal(this)">
                                Upload Berkas
                            </button>
                        @else
                            ...
                            @if ($is_kredit_page)
                                @if (!$item->bukti_pembayaran->is_confirm)
                                    <a style="cursor: pointer; text-decoration: underline;"
                                        class="confirm-bukti-pembayaran toggle-modal-confirm-bukti-pembayaran text-red-600"
                                        data-target-id="modalConfirmBuktiPembayaran" data-id-category="1"
                                        ...
                                        onclick="showModal(this)" href="#">Konfirmasi Bukti Pembayaran</a>
                            ...
                            @if ($is_kredit_page)
                                @if (\Session::get(config('global.user_role_session')) == $staf_analisa_kredit_role)
                                    <button class="toggle-modal-upload-bukti-pembayaran underline text-red-600"
                                        data-kategori="{{ $item->kategori }}"
                                        data-target-id="modalUploadBuktiPembayaran" data-id_kkb="{{ $item->kkb_id }}"
                                        onclick="showModal(this)">
                                        Bayar
                                    </button>
                                @else
                                    ...
                                    @if (!$item->bukti_pembayaran->is_confirm)
                                        @if (\Session::get(config('global.role_id_session')) == 3)
                                            <button class="toggle-modal-confirm-bukti-pembayaran text-red-600"
                                                style="cursor: pointer;" data-target-id="modalConfirmBuktiPembayaran"
                                                data-file="{{ $item->bukti_pembayaran->file }}"
                                                data-kategori="{{ $item->kategori }}"
                                        ...
                                        @else
                                            <button class="toggle-modal-bukti-pembayaran underline"
                                                style="cursor: pointer;" data-target-id="modalBuktiPembayaran"
                                                data-file="{{ $item->bukti_pembayaran->file }}"
                                                data-kategori="{{ $item->kategori }}"
                                                data-tanggal="{{ $item->bukti_pembayaran->date }}"
                                                data-confirm="{{ $item->bukti_pembayaran->is_confirm }}"
                        ...
                        @if ($item->penyerahan_unit)
                                    ...
                                    @if (isset($item->penyerahan_unit->is_confirm))
                                        @if ($item->penyerahan_unit->is_confirm)
                                            <a style="text-decoration: underline; cursor: pointer;"
                                                ...
                                                data-id-category="2" data-kategori="{{ $item->kategori }}"
                                        ...
                                        @endif
                                    @else
                                        <a style="text-decoration: underline; cursor: pointer;" class="toggle-modal"
                                            data-target-id="modalConfirmPenyerahanUnit" data-id-category="2"
                                            data-kategori="{{ $item->kategori }}"
                                            data-id-doc="{{ $item->penyerahan_unit ? $item->penyerahan_unit->id : 0 }}"
                                            data-file="@isset($item->penyerahan_unit->file){{ $item->penyerahan_unit->file }}@endisset"
                                            data-confirm="{{ $item->penyerahan_unit->is_confirm }}"
                            ...
                            @elseif(
                                \Session::get(config('global.role_id_session')) == 2 &&
                                    \Session::get(config('global.user_role_session')) == $staf_analisa_kredit_role)
                                @if (isset($item->penyerahan_unit->is_confirm))
                                    @if ($item->penyerahan_unit->is_confirm)
                                        <a style="text-decoration: underline; cursor: pointer;" class="toggle-modal"
                                            data-target-id="modalConfirmPenyerahanUnit" data-id-category="2"
                                    ...
                                    @else
                                        <a style="text-decoration: underline; cursor: pointer;"
                                            ...
                                            data-tanggal="{{ date('d-m-Y', strtotime($item->penyerahan_unit->date)) }}"
                                            data-confirm_at="{{ $item->penyerahan_unit->confirm_at ? date('d-m-Y', strtotime($item->penyerahan_unit->confirm_at)) : '-' }}"
                                            href="#" onclick="showModal(this)">Konfirmasi</a>
                                    @endif
                                @endif
                            @else
                                @if (property_exists($item->penyerahan_unit, 'is_confirm'))
                                    ...
                                    @else
                                        <a style="text-decoration: underline; cursor: pointer;" class="toggle-modal"
                                            ...
                                            data-tanggal="{{ date('d-m-Y', strtotime($item->penyerahan_unit->date)) }}"
                                            data-confirm_at="{{ $item->penyerahan_unit->confirm_at ? date('d-m-Y', strtotime($item->penyerahan_unit->confirm_at)) : '-' }}"
                                            href="#" onclick="showModal(this)">Menunggu konfirmasi cabang</a>
                                    @endif
                                @else
                                    <a style="text-decoration: underline; cursor: pointer;" class="toggle-modal"
                                        data-target-id="modalConfirmPenyerahanUnit" data-id-category="2"
                                ...
                                @if ($item->penyerahan_unit->is_confirm)
                                    @if ($item->imbal_jasa)
                                        @if (!$item->imbal_jasa->is_confirm)
                                            ...
                                            @endphp
                                            <a style="cursor: pointer; text-decoration: underline;"
                                                class="confirm-imbal-jasa toggle-modal-confirm-imbal-jasa text-red-600"
                                                ...
                                                data-confirm="{{ $item->imbal_jasa->is_confirm }}" href="#"
                ...
                @else
                    @if ($item->imbal_jasa)
                        ...
                        @elseif ($item->imbal_jasa->is_confirm)
                            <a class="bukti-pembayaran-modal toggle-modal-confirm-imbal-jasa"
                                style="cursor: pointer; text-decoration: underline;"
                                data-target-id="modalConfirmImbalJasa" data-kategori="{{ $item->kategori }}"
                                data-confirm="{{ $item->imbal_jasa->is_confirm }}"
                                data-tanggal="{{ \Carbon\Carbon::parse($item->imbal_jasa->date)->format('d-m-Y') }}"
                                ...
                                data-file="{{ $item->imbal_jasa->file }}" onclick="showModal(this)">Selesai</a>
                        @else
                            Menunggu Pembayaran dari Cabang
                        @endif
                    @else
                                    ...
                                    @if ($is_kredit_page)
                                        @if (\Session::get(config('global.user_role_session')) == $staf_analisa_kredit_role)
                                            ...
                                            <a href="#" style="text-decoration: underline; cursor: pointer;"
                                                ...
                                                data-target-id="modalUploadImbalJasa" data-id="{{ $item->id }}"
                                                data-kategori="{{ $item->kategori }}"
                                                data-nominal="Rp {{ $nominal_imbal_jasa }}"
                                ...
                                @if ($item->polis)
                                    @if ($item->polis['file'] && $item->polis['is_confirm'])
                                        <a style="text-decoration: underline; cursor: pointer;"
                                            ...
                                            data-target="#detailPolis" data-file="{{ $item->polis['file'] }}"
                                ...
                                @if ($item->stnk)
                                    @if ($item->stnk['file'] && $item->stnk['is_confirm'])
                                        <a style="text-decoration: underline; cursor: pointer;"
                                            ...
                                            data-target="#detailStnk" data-file="{{ $item->stnk['file'] }}"
                                ...
                                @if ($item->bpkb)
                                    @if ($item->bpkb['file'] && $item->bpkb['is_confirm'])
                                        <a style="text-decoration: underline; cursor: pointer;"
                                            ...
                                            data-target="#detailBpkb" data-file="{{ $item->bpkb['file'] }}"
        ...
        <td class="text-center">
            <span
                class="@if (strtolower($item->status) == 'done') text-blue-600 @else text-grey @endif">{{ ucwords($item->status) }}</span>
        </td>
        {{--  Aksi  --}}
        <td>
            @if ($is_kredit_page)
                @if (strtolower($item->status) == 'done')
                    <button class="px-4 py-2 bg-theme-btn/10 rounded text-theme-btn toggle-modals"
                        data-kategori="{{ $item->kategori }}" data-target-id="modalDetailPo"
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu  right-20">
                            @if ($item->tgl_ketersediaan_unit)
                                @if ($item->bukti_pembayaran)
                                    @if (
                                        !$item->penyerahan_unit &&
                                            $item->bukti_pembayaran->is_confirm &&
                                            \Session::get(config('global.role_id_session')) == 3)
                                        <li>
                                            ...
                                            @if (!isset($item->stnk['file']) || !isset($item->polis['file']) || !isset($item->bpkb['is_confirm']))
                                                ...
                                                <li>
                                                    <a class="item-dropdown toggle-modal"
                                                        ...
                                                        data-berkas-unggah="{{ $item->is_upload_kkb === null ? 'vendor' : $item->is_upload_kkb }}"
                                                        data-id_kkb="{{ $item->kkb_id }}"
                            ...
                            @endif
                            <li>
                                <a class="item-dropdown toggle-modals" data-target-id="modalDetailPo"
                                    data-id="{{ $item->id }}" data-kategori="{{ $item->kategori }}"
