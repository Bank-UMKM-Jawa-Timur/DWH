                        ...
                        <div class="input-box space-y-3 col-span-1">
                            ...
                            <select name="status" id="status" class="w-full p-2 border" required>
                                ...
                                @foreach ($allStatus as $item)
                                    <option value="{{$item}}" @if(\Request::has('status')){{$item == \Request::get('status') ? 'selected' : ''}}@endif>{{ $item }}</option>
                                @endforeach
                            </select>
                            <small class="form-text text-red-600 error"></small>
                        </div>
                ...
                <div class="table-wrapper bg-white border rounded-md w-full p-2 mt-5">
                    ...
                    <div class="tables mt-2">
                        <table class="table-auto w-full">
                            ...
                            <tbody>
                                @forelse ($pengajuan_klaim as $item)
                                    <tr>
                                        <td>{{$loop->iteration}}</td>
                                        <td>{{$item->dataNasabah['nama']}}</td>
                                        <td>{{$item->no_aplikasi}}</td>
                                        <td>{{$item->no_rek}}</td>
                                        <td>{{date('d-m-Y', strtotime($item->dataNasabah['tanggal']))}}</td>
                                        <td>{{$item->no_polis}}</td>
                                        <td>{{$item->no_pk}}</td>
                                        <td>{{$item->status}}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="10">Belum ada data pelunasan.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    <div class="footer-table p-3 text-theme-text lg:flex lg:space-y-0 space-y-10 justify-between">
                        <div class="w-full">
                            <div class="pagination kkb-pagination">
                                @if ($pengajuan_klaim instanceof \Illuminate\Pagination\LengthAwarePaginator)
                                    {{ $pengajuan_klaim->links('pagination::tailwind') }}
                                @endif
                            </div>
                        </div>
                    </div>
