            ...
            <div>
                <span class="relative z-0 inline-flex shadow-sm rounded-md">
                    ...
                    @if ($paginator->onFirstPage())
                        <button class="flex items-center justify-center w-10 h-10 text-black-600 transition-colors duration-150 rounded-full focus:shadow-outline hover:bg-black-100">
                    ...
                    @else
                        <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="flex items-center justify-center w-10 h-10 text-black transition-colors duration-150 rounded-full focus:shadow-outline hover:bg-green-100" aria-label="{{ __('pagination.previous') }}">
                            <svg class="w-4 h-4 fill-current" viewBox="0 0 20 20"><path d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" fill-rule="evenodd"></path></svg>
                        </a>
                    @endif
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <button class="w-10 h-10 text-white transition-colors duration-150 bg-theme-primary border border-r-0 border-theme-primary rounded-full focus:shadow-outline">{{ $page }}</button>
                                    </span>
                                @else
